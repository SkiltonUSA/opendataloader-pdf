...
Benchmark-backed tip: if you only need a subset of pages, merge consecutive page numbers into the shortest possible pages argument before calling convert(). For RAG pipelines, prefer Markdown or JSON+Markdown output and enable tagged-PDF structure when it is available.
